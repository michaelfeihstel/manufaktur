class ContactFiltersController < ApplicationController
  ...
  def set_product_family_share_chart
    @contact = Contact.find(params[:id])
    from = params[:from] || "01-01-0001".to_datetime
  ...
  def set_order_stats_panel
    params[:from] ||= "01-01-0001".to_datetime
    params[:to] ||= Time.now

    @contact = Contact.find(params[:id])
    @orders = @contact.orders.where(created_at: params[:from]..params[:to])

    render "contacts/ajax/set_order_stats_panel"
  end
end