...
class Contact < ApplicationRecord
  ...
  # methods
  def billing_address
    addresses.first.try(:full_address)
  end

  def employee?
    try(:contact_role).try(:name) == 'Mitarbeiter'
  end

  def retailer?
    try(:contact_role).try(:name) == 'retailer'
  end

  def supplier?
    try(:contact_role).try(:name) == 'Supplier'
  end
end
